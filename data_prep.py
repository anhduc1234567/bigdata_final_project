...
parser.add_argument('--dst_folder_points', type=str, help='Directory in which points.txt and clusters.txt will be saved.')
...
def main(src_folder, dst_folder_points,dst_folder_clusters,dst_img, k):
    # create directory
    # Path(dst_folder).mkdir(parents=True, exist_ok=True)
    ...
    for i in range(len(img_files)):
        ...
        img_new_path = join(dst_img, f'tem_img{i}.png')
        cover_img(clusters_path,src_img_path,img_new_path)

if __name__ == '__main__':
    args = parser.parse_args()
    main(args.src_img, args.dst_folder_points,args.dst_folder_clusters,args.dst_img, args.k_init_centroids)
